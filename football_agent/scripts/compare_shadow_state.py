...
def _is_stale_unsent_no_bet_state(item: Dict[str, Any], since_utc: str) -> bool:
    """Return True for old local NO_BET notification-cache rows that predate compare window.

    notification_state.json is restored from GitHub cache and can contain old
    sent=false NO_BET rows. Those rows are local notification hygiene, not
    current pick parity. If the row was last updated before SHADOW_COMPARE_SINCE_UTC,
    it must not force a critical database state mismatch.
    """
    if not since_utc:
        return False
    ...
    if status != "NO_BET":
        return False
    if not _is_falsey(item.get("sent")):
        return False
    updated_at = str(item.get("updated_at_utc") or "").strip()
    if not updated_at:
        return False
    return updated_at < since_utc


def _filter_stale_unsent_no_bet_state(
    state: Dict[str, Dict[str, Any]],
    since_utc: str,
) -> Dict[str, Dict[str, Any]]:
    if not since_utc:
    ...
    return {
        ...
        if not _is_stale_unsent_no_bet_state(value, since_utc)
    }
...
def main() -> None:
    ...
    local_state_current = _filter_stale_unsent_no_bet_state(local_state_all, compare_since_utc)
    local_state = _filter_state_to_eligible_keys(local_state_current, eligible_state_keys)
    db_state_all = {
    ...
    report = {
        ...
        "local_notification_states_after_stale_filter": len(local_state_current),
